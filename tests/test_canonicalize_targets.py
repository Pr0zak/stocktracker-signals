...
def test_the_real_plan_collapses_to_five_groups():
    ...
    assert _as_pairs(note) == [("US_EQUITY", 40.0), ("VXUS", 22.0),
                               ("XOM", 10.0), ("AMZN", 8.0), ("BTC", 5.0)]
...
def test_ticker_labels_are_resolved_to_their_group():
    ...
    canonicalize_targets(note, group_of=_exposure_group)
    assert _as_pairs(note) == [("US_EQUITY", 35.0), ("VXUS", 25.0), ("BTC", 15.0)]


# ---------------------------------------------------------------- it must not invent changes

def test_a_clean_plan_is_left_exactly_alone():
    note = _note([("US_EQUITY", 25.0), ("VXUS", 25.0), ("SCHD", 20.0), ("AMZN", 15.0)])
    assert canonicalize_targets(note, group_of=_exposure_group) == []
    assert _as_pairs(note) == [("US_EQUITY", 25.0), ("VXUS", 25.0),
                               ("SCHD", 20.0), ("AMZN", 15.0)]


def test_genuinely_distinct_groups_are_not_merged():
    """VXUS (0.778 vs VTI) and SCHD (0.641) do real diversification work and keep their own groups."""
    note = _note([("US_EQUITY", 25.0), ("VXUS", 25.0), ("SCHD", 25.0), ("GOLD", 10.0)])
    canonicalize_targets(note, group_of=_exposure_group)
...
def test_order_of_first_appearance_is_kept():
    ...
    canonicalize_targets(note, group_of=_exposure_group)
    assert [g for g, _ in _as_pairs(note)] == ["VXUS", "US_EQUITY", "AMZN"]
...
def test_the_vocabulary_never_offers_a_retired_label():
    ...
    assert set(vocab) == {"US_EQUITY", "VXUS"}
